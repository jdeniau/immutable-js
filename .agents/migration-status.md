...
- **R3** — `reverseFactory` reverse-iteration keys (`?? 0` from PR #2206, `NaN`
  already in v5): **fixed** — the keys now count down from the number returned
  by `ensureSize(collection)` (the reversed sequence has the same size as its
  source). Regression tests cover both `__iterate` (via `reduceRight`) and
  `__iterator` (via a keyed reverse) in `__tests__/IndexedSeq.ts`; CHANGELOG
  entry since the `NaN` behaviour shipped in v5.
